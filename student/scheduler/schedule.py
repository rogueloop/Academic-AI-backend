...
class StudyScheduleEnvironment:
    def __init__(self, daily_time_quota, task_pool):
        self.daily_time_quota = daily_time_quota*60
        self.state = daily_time_quota*60
...
# ]
    ...
    def step(self, action):
        ...
        if task['subject'] in self.completed_topics:
            ...
            # You can adjust the threshold for a close deadline(in days) example(20 days)
            if task['deadline'] > 20:
                pass
            else:
        ...
        if  task['focus_area'] and  self.num_focus_areas_selected > self.max_focus_areas:
            return self.state, 0  # Can't study more focus areas than the maximum allowed

        task_time = np.random.randint(
            task['min_study_time'], task['max_study_time'] + 1)
        reward = 0
        ...
        if task_time <= self.state:
            ...
            self.num_focus_areas_selected = self.num_focus_areas_selected+1
...
class QNetwork(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc1 = nn.Linear(input_size, 312)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(312, output_size)
    ...
    def train(self):
        ...
        # Loop until the maximum number of steps is reached or the episode is done
            ...
            # Get the current Q-value for the selected action
                current_q_value = self.q_network(
                    torch.tensor([state], dtype=torch.float32))[action]

            # Compute the loss between the current Q-value and the target Q-value
                loss = self.criterion(
                    current_q_value, torch.tensor(target_q_value))
        ...
        # Print the total reward obtained in this epoch
            print(
                f'Epoch {epoch + 1}/{self.num_epochs}, Total Reward: {total_reward}')
...
class Scheduler:
    ...
    def __init__(self, daily_time_quota, learning_rate, gamma, epsilon, num_epochs, task_pool):
        ...
        self.agent = QLearningAgent(
            self.env, learning_rate, gamma, epsilon, num_epochs)
...
# Train the agent
        ...
        print(
            f"Training completed successfully. Training Time: {training_time} seconds")
    ...
    def generate_study_schedule(self):
        ...
        # Check if the task has already been selected
            ...
            if task_time <= state:
                schedule.append(
                    {'Subject': task['subject'], 'Task': task['task_name'], 'StudyTime': task_time,'focus_area':task['focus_area']})
                state -= task_time
            # Add the index to the set of selected tasks
                selected_tasks.add(action)
            if task_time > state and task_time > 15 and action not in selected_tasks:
                schedule.append(
                {'Subject': task['subject'], 'Task': task['task_name'], 'StudyTime': task_time,'focus_area': task['focus_area']})
                state -= task_time
                selected_tasks.add(action)
        ...
        for entry in schedule:
            print(
                f"Subject: {entry['Subject']}, Task: {entry['Task']}, Study Time: {entry['StudyTime']} minutes")
        return schedule
